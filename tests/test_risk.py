...
class TestRiskClassifier(unittest.TestCase):
    def test_low_risk(self):
        ...
        self.assertEqual(classify_risk(1.99), LOW)
        self.assertEqual(classify_risk(-1.5), LOW)

    def test_medium_risk(self):
        self.assertEqual(classify_risk(2.0), MEDIUM)
        self.assertEqual(classify_risk(3.0), MEDIUM)
        self.assertEqual(classify_risk(5.0), MEDIUM)
        self.assertEqual(classify_risk(-4.0), MEDIUM)

    def test_high_risk(self):
        self.assertEqual(classify_risk(5.01), HIGH)
        self.assertEqual(classify_risk(6.0), HIGH)
        self.assertEqual(classify_risk(-10.0), HIGH)
        self.assertEqual(classify_risk(50.0), HIGH)

    def test_custom_thresholds(self):
        self.assertEqual(
            classify_risk(3.0, low_max_pct=5.0, medium_max_pct=10.0),
            LOW,
        )
        self.assertEqual(
            classify_risk(15.0, low_max_pct=5.0, medium_max_pct=10.0),
            HIGH,
        )
